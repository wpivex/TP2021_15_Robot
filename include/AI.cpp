...
void AI::trackObjectsForCurrentFrame(BaseRobot *robot, vision *camera, std::vector<GoalPosition> &goals, int targetID) {
...
int AI::findGoalID(std::vector<GoalPosition> &goals) {
...
int AI::detectionAndStrafePhase(BaseRobot *robot, vision *camera, float *horizonalDistance, int matchStartTime) {
...
float AI::getDistanceFromArea(int area) {
...
GoalPosition* AI::getGoalFromID(std::vector<GoalPosition> &goals, int targetID) {
  for (int i = 0; i < goals.size(); i++) {
...
void AI::runAI(BaseRobot *robot, int matchStartTime) {
